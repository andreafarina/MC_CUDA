...
__device__ unsigned short int FindDetector(float distance);
...
//unsigned long long int pos; //posizione salvataggio fotone
	...
	PhotonStruct p = DeviceMem.p[begin+tx];
 	...
 	//for(short int i=0;i<MAX_DETECTORS;i++)
	//  count_det[i]=0;
				...
				if(Reflect(&p,new_layer,&x,&a)==0u){
					...
					if(new_layer == 0){
						...
						if (RorT_dc=='R'){ 
							...
							//i_det=-1;
							//distance=sqrtf(p.x*p.x+p.y*p.y);
							i_det=FindDetector(sqrtf(p.x*p.x+p.y*p.y));
							//Binary saerch for detectors_dc
							//unsigned int Mmin=0;
							//unsigned int Mmax=n_detectors_dc[0];
							//unsigned int mid=floorf((Mmin+Mmax)/2);
							//Mmin=0u;
//							Mmax=n_detectors_dc[0];
//							mid=floorf((Mmin+Mmax)/2);
//							while (!((distance>=detectors_dc[mid+mid-2]) && (distance<=detectors_dc[mid+mid-1]))){
//							if (distance>detectors_dc[mid+mid-2])
//								Mmin=mid+1;
//							else
//								Mmax=mid-1;
//							mid=floorf((Mmax+Mmin)/2);
//							}
//							i_det=mid-1;
							...
							if (i_det>-1){
								...
								if (DeviceMem.photons[i_det]<N_photons_dc[0]){//N_photons_dc[0]){//occhio!rischio di trascurare gia fotoni utili
									...
									for(i_lay=0;i_lay<n_layers_dc[0];i_lay++){
					...
					if(new_layer > *n_layers_dc){
						...
						if (RorT_dc=='T'){
							...
							// for(short int i=n_detectors_dc[0]-1;(i>-1)&&(detected=='0');i--){
...
//							i_det=mid-1;
 	...
 	}//while(term_photons_in_thread<n_detectors_dc[0]*NUMSTEPS_GPU);
...
__device__ unsigned short int FindDetector(float distance) //Return -1 if the photon doesn't hit the detector, return detector number 0,1,2,...
{
	char detected='0';
